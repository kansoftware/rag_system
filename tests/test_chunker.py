...
def chunker():
    ...
    return MarkdownChunker(
        chunk_size=100, chunk_overlap=20, model_name="distilbert-base-uncased"
    )


def test_simple_chunking(chunker):
    text = (
        ("Это первое предложение. " * 20)
        + ("Это второе предложение. " * 20)
        + ("Это третье. " * 20)
    )
...
def test_header_separation(chunker):
    text = (
        "## Секция 1\nТекст первой секции, достаточно длинный, чтобы быть уверенным, что он не сольется со второй. "
        * 10
        + "\n\n## Секция 2\nТекст второй секции."
    )
    chunks = chunker.chunk(text, {})
