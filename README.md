...
### 3. Установка для разных GPU

Проект поддерживает два режима установки зависимостей для GPU: для современных видеокарт (CUDA 12+) и для устаревших (CUDA 11.7, архитектура `sm_61`).

-   **Для современных GPU (рекомендуется):**
    ```bash
    pip install -e ".[dev,gpu-modern]"
    ```

-   **Для устаревших GPU:**
    Вам потребуется установить `onnxruntime-gpu` и `torch` с поддержкой CUDA 11.7.
    ```bash
    pip install -e ".[dev,gpu-legacy]" --extra-index-url https://download.pytorch.org/whl/cu117
    ```
    После установки, откройте файл `.env` и установите `RERANKER_ONNX=True`, чтобы активировать использование ONNX для ре-ранжирования.
