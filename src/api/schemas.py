...
class QueryRequest(BaseModel):
    ...
    top_k_initial: int = Field(30, ge=1, le=100, description="Количество кандидатов для αρχικού поиска")
    top_k_final: int = Field(7, ge=1, le=20, description="Количество финальных источников после ре-ранжирования")
    domain_filter: Optional[str] = Field(None, description="Фильтр по домену источников")
    min_confidence: float = Field(0.7, ge=0.0, le=1.0, description="Минимальный порог уверенности для ответа")
